...
async def _fetch_candles_df(engine: NexusEngine, asset: str, timeframe_min: int, rows: int) -> 'pd.DataFrame':  # type: ignore
    """Login if needed, pull candles via broker adapter, and return a pandas DataFrame."""
    ok = await engine.login_broker()
    if not ok:
        raise RuntimeError("Failed to login to Quotex; provide QUOTEX_EMAIL/QUOTEX_PASSWORD in env or settings")
    ...
    try:
        ...
        pass
    if getattr(engine, "_broker", None) is None:
        raise RuntimeError("Broker adapter unavailable after login")
    candles = await engine._broker.get_candles_async(asset, int(timeframe_min) * 60, max(100, int(rows)))  # type: ignore[attr-defined]
    if not candles:
...
async def main():
    ...
    parser.add_argument("--live-backtest", action="store_true", help="Run backtest on live OHLC via Quotex (uses demo/practice mode)")
    # Place a single trade on the broker (uses demo if --demo is set)
    parser.add_argument("--place-trade", action="store_true", help="Place a single trade on Quotex (demo if --demo)")
    ...
    parser.add_argument("--duration", type=int, default=60, help="Expiration in seconds for --place-trade (default 60)")
    # Minimal live trading loop (demo/practice by default)
    parser.add_argument("--live-trade", action="store_true", help="Run a minimal live trading loop (demo by default)")
    parser.add_argument("--max-trades", type=int, default=3, help="Max trades for --live-trade")
    parser.add_argument("--interval", type=int, default=30, help="Seconds between signal checks for --live-trade")
    ...
    parser.add_argument("--aggregate-reports", action="store_true", help="Aggregate backtest reports and show leaderboard")
    parser.add_argument("--force-epsilon", type=float, help="Force exploration epsilon (0-1) during backtest")
    parser.add_argument("--evolve", action="store_true", help="Run evolutionary optimization over ensemble weights")
    ...
    if len(args.assets) == 1 and args.assets[0].lower() == 'auto':
        console.print(Panel("Live asset auto-fetch not implemented in lightweight prototype", style="yellow"))
    ...
    if args.aggregate_reports:
        ...
        for idx, e in enumerate(entries, start=1):
            table.add_row(
                str(idx), e.asset, f"{e.score:.4f}", str(e.total_trades), f"{e.total_profit:.2f}",
                f"{e.win_rate*100:.1f}%", f"{e.max_drawdown:.2f}", f"{e.profit_factor:.2f}"
            )
        console.print(table)
        sys.exit(0)
    ...
    if args.live_backtest:
        try:
            ...
            df = await _fetch_candles_df(engine, asset=asset, timeframe_min=args.timeframe, rows=args.rows)
            ...
            result = await bt.run(meta, engine, df, asset=asset, timeframe=args.timeframe, mode="market")
    ...
    if args.evolve:
        ...
        for gr in generations:
            ...
            table.add_row(str(gr.generation), f"{gr.best_fitness:.4f}", w_preview + (" ..." if len(gr.best_weights) > 3 else ""))
        console.print(table)
        console.print(Panel(f"Final Best Weights: {best}", style="green"))
        sys.exit(0)

    if args.start:
        console.print(Panel("Trading loop not implemented in lightweight prototype.", style="yellow"))
        sys.exit(0)
