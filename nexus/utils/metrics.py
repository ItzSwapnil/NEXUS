...
def sharpe_ratio(
    returns: Union[pd.Series, np.ndarray], risk_free_rate: float = 0.0, period: str = "daily"
) -> float:
...
def sortino_ratio(
    returns: Union[pd.Series, np.ndarray], target_return: float = 0.0, period: str = "daily"
) -> float:
...
def conditional_value_at_risk(
    returns: Union[pd.Series, np.ndarray], confidence_level: float = 0.05
) -> float:
